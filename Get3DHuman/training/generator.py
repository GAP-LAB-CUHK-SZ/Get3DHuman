...
class I3DGAN_Generator(torch.nn.Module):
    def __init__(self,
        ...
        **synthesis_kwargs,         # Arguments for SynthesisNetwork.
        ...
        #                                 march_step=50, buffer_size=buffer_size, 
        ...
        # self.Dis_texture = DisBackbone(self.c_dim, img_resolution=256, img_channels=3)
    ...
    # def forward_shape(self, z, c, truncation_psi=1, truncation_cutoff=None, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
    ...
    #     self.feature_shape = self.synthesis_shape(ws_shape, c, update_emas=update_emas, neural_rendering_resolution=neural_rendering_resolution, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, **synthesis_kwargs)

    #     self.feature_volume_shape, _ = self.filter_shape(self.feature_shape)
        
    #     return self.feature_volume_shape[0]
    ...
    # def render_depth(self,z_shape, c)
    #     with torch.no_grad():
    ...
    #         self.feature_volume_shape = self.synthesis_shape(ws_shape, c, update_emas=update_emas, neural_rendering_resolution=None, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, **synthesis_kwargs)
    
    #     return detph, points
    # def DR_depnorm(self, R, T, calib, feat_list):
        
    #     grad_settings={"depth": True, "normal": False, "silhouette": False}
        
    #     latent_pifu = feat_list[0]
    ...
    #     depth_rendered, min_points, normal_rendered, valid_mask_rendered = render_output
    
    #     return depth_rendered, min_points#, normal_rendered, valid_mask_rendered
    ...
    def forward_DR(self, z_shape, c, R, T, calib, cam_x, cam_y, cuda, spatial_enc, not_train_DR, truncation_psi=1, truncation_cutoff=None,  update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        with torch.no_grad():
             ...
             self.feature_volume_shape_dr, _ = self.filter_shape(self.feature_shape_dr.repeat([1,3,1,1]))
    ...
    def forward_texture(self, z_texture, c, shape_feature, min_points, calib, spatial_enc, truncation_psi=1, truncation_cutoff=None,  update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        self.feature_texture = self.synthesis_texture(ws_shape, c, update_emas=update_emas, neural_rendering_resolution=None, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, **synthesis_kwargs)
        self.feature_volume_texture, _ = self.filter_texture(torch.cat([self.feature_texture, shape_feature[0]],1))
        
        feature_volume_cat = torch.cat([self.feature_volume_texture[0], shape_feature[1]],1)
    ...
    # def forward_texture_(self, z_texture, z_shape, c, R, T, calib, cam_x, cam_y, cuda, spatial_enc, not_train_DR, truncation_psi=1, truncation_cutoff=None,  update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        
    #     # Render shape depth, and find the points.
    #     with torch.no_grad():
    #          ws_shape = self.mapping_shape(z_shape, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
    #          self.feature_shape_dr = self.synthesis_shape(ws_shape, c, update_emas=update_emas, neural_rendering_resolution=None, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, **synthesis_kwargs)
    #          self.feature_volume_shape_dr, _ = self.filter_shape(self.feature_shape_dr)
             
    #     # self.cam_x, self.cam_y = torch.tensor(cam_x).to(device=cuda), torch.tensor(cam_y).to(device=cuda)
    #     self.cache_image = torch.ones([z_shape.size(0), self.render_size*self.render_size]).to(device=self.cuda) >2
    #     render_output = self.sdf_renderer.render_ortho_points(cam_x, cam_y, [self.feature_shape_dr[0], calib, orthogonal, spatial_enc, index, self.cache_image],
    #                                                R, T, no_grad=not_train_DR,  sample_index_type='min_abs', no_grad_depth=not_train_DR, no_grad_normal=True)
    ...
    #     ws_shape = self.mapping_texture(z_texture, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
    #     self.feature_texture = self.synthesis_texture(ws_shape, c, update_emas=update_emas, neural_rendering_resolution=None, cache_backbone=cache_backbone, use_cached_backbone=use_cached_backbone, **synthesis_kwargs)
    #     self.feature_volume_texture, _ = self.filter_texture(torch.cat([self.feature_texture, self.feature_shape_dr],1))
        
        
    #     pred_texture = self.decode_texture(self.mlp_texture, [self.feature_volume_texture[0], calib, orthogonal, spatial_enc, index, self.cache_image], min_points).squeeze(-1)
    #     pred_texture = pred_texture.reshape(-1, 3, self.render_size, self.render_size)
        
    #     return pred_texture, depth_rendered, min_points
