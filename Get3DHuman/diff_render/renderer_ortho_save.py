...
class SDFRenderer(object):
    def __init__(self, cuda,
                 decoder, intrinsic, img_hw=None, transform_matrix=None, march_step=50, buffer_size=5, ray_marching_ratio=1.5, use_depth2normal=False, max_sample_dist=0.2, radius=1.0, threshold=5e-5, scale_list=[4, 2, 1], march_step_list=[3, 3, -1], use_gpu=True, is_eval=True):
...
##        cam_pos_ortho = self.get_camera_location_ortho(R)
#        cam_rays = self.get_camera_rays(R)
#        
#        dist = self.get_distance_from_origin(cam_pos, cam_rays)
...
##        init_zdepth, valid_mask = self.get_intersections_with_unit_spheres_ortho(cam_pos_ortho, cam_rays,cam_pos)
...
#        print(init_zdepth.shape)
        ...
        # ray marching
        sdf_list, marching_zdepth_list, points_list, valid_mask_render = self.ray_marching_ortho(cam_pos_ortho, cam_rays, R, init_zdepth, valid_mask, latent, clamp_dist=clamp_dist, no_grad=no_grad_camera, ray_marching_type=ray_marching_type, use_transform=use_transform)

        profiler.report_process('[DEPTH] ray marching time')


        # get differnetiable samples
        min_sdf_sample = self.get_min_sdf_sample_ortho(sdf_list, points_list, latent, index_type='min_abs', clamp_dist=clamp_dist, profile=profile, no_grad=no_grad_mask)
...
#              ray_marching_recursive(self, cam_pos,               cam_rays, init_zdepth, valid_mask, latent, march_step=None, stop_threshold=None, clamp_dist=0.1, no_grad=False, use_transform=True, use_first_query_check=True)
    def ray_marching_recursive_ortho(self, cam_pos_ortho, cam_pos, cam_rays, init_zdepth, valid_mask, latent, march_step=None, stop_threshold=None, clamp_dist=0.1, no_grad=False, use_transform=True, use_first_query_check=True):
        ...
        maxbound_zdepth = self.get_maxbound_zdepth_ortho(cam_pos_ortho, cam_pos, cam_rays)
...
#            print(valid_cam_rays.shape)
    ...
    def get_min_sdf_sample_ortho(self, sdf_list, points_list, latent, index_type='min_abs', clamp_dist=0.1, profile=False, no_grad=False):
        profiler = Profiler(silent = not profile)
        _, index = self.get_index_from_sdf_list(sdf_list, 1, index_type=index_type)
        points = self.collect_data_from_index(points_list, index)[0] # (N, 3)
        min_sdf_sample = decode_sdf(self.decoder, latent, points, clamp_dist=None, no_grad=no_grad).squeeze(-1)
