...
from diff_render.decoder_utils import decode_sdf, decode_sdf_gradient
from diff_render.profiler import Profiler
from diff_render.render_utils import depth2normal
import copy
import time
...
#    
    ...
    def render_ortho(self, latent, R, T, clamp_dist=0.1, sample_index_type='min_abs', profile=False, no_grad=False, no_grad_depth=False, no_grad_normal=False, no_grad_mask=False, no_grad_camera=False, normalize_normal=True, use_transform=True, ray_marching_type='pyramid_recursive', num_forward_sampling=0):
        '''
        ...
        # render depth
        Zdepth, valid_mask, min_abs_query = self.render_depth_ortho(latent, R, T, clamp_dist=clamp_dist, sample_index_type=sample_index_type, profile=profile, no_grad=no_grad, no_grad_depth=no_grad_depth, no_grad_mask=no_grad_mask, no_grad_camera=no_grad_camera, ray_marching_type=ray_marching_type, use_transform=use_transform) # (H*W), (H*W), (H*W)
...
#            normal = self.render_normal(latent, R, T, Zdepth, valid_mask, clamp_dist=clamp_dist, no_grad=no_grad_normal, normalize=normalize_normal, use_transform=use_transform) # (3, H*W)
            normal = self.render_normal_ortho(latent, R, T, Zdepth, valid_mask, clamp_dist=clamp_dist, no_grad=no_grad_normal, normalize=normalize_normal, use_transform=use_transform) # (3, H*W)
            
            normal = torch.matmul(R, normal) # (3, H*W)
            normal[2,:] = normal[2,:].clone() * (-1) # transformed the direction to align with rendering engine (left-hand sided).
#            normal[0,:] = normal[0,:].clone() # transformed the direction to align with rendering engine (left-hand sided).
...
#            cam_pos_dist = self.cam_pos_dist
    ...
    def render_normal_ortho(self, latent, R, T, Zdepth, valid_mask, clamp_dist=0.1, MAX_POINTS=100000, no_grad=False, normalize=True, use_transform=True):
...
#        cam_rays = self.get_camera_rays(R)
        ...
        #20211012
        valid_cam_pos_ortho = cam_pos_ortho[:, valid_mask]
        
        Znormal.requires_grad = True
#        generate_point_samples_ortho
        # compute normal
#        points = self.generate_point_samples(cam_pos, valid_cam_rays, valid_zdepth, has_zdepth_grad=False, inv_transform=use_transform)
        points = self.generate_point_samples_ortho(valid_cam_pos_ortho, cam_pos, valid_cam_rays, valid_zdepth, has_zdepth_grad=False, inv_transform=use_transform)
...
#        valid_normal = valid_normal_untransformed
...
#        Znormal[:,:,:] = Znormal[:,:,:]
    ...
    def render_depth_ortho(self, latent, R, T, clamp_dist=0.1, sample_index_type='min_abs', profile=False, no_grad=False, no_grad_depth=False, no_grad_mask=False, no_grad_camera=False, ray_marching_type='recursive', use_transform=True):
        if no_grad:
...
#        cam_pos_ortho, cam_rays = sdf_renderer.get_rays_ortho(R) 
#        dist = sdf_renderer.get_distance_from_origin_ortho(cam_pos_ortho, cam_rays)
#        
...
#        valid_mask[valid_mask] = valid_mask_render
        #20211017
#        valid_mask = min_sdf_sample < self.mask_threshold
#        mask_pifu = min_sdf_sample<0.001
...
#              ray_marching_recursive(self, cam_pos,               cam_rays, init_zdepth, valid_mask, latent, march_step=None, stop_threshold=None, clamp_dist=0.1, no_grad=False, use_transform=True, use_first_query_check=True)
    ...
    def ray_marching_ortho(self, cam_pos_ortho, cam_pos, R, init_zdepth, valid_mask, latent, march_step=None, clamp_dist=0.1, no_grad=False, use_transform=True, ray_marching_type='recursive', split_type='raydepth'):
        '''
        ...
        if ray_marching_type == 'trivial_non_parallel':
            cam_rays = self.ray_marching_recursive_ortho(R)
            return self.ray_marching_trivial_non_parallel_ortho(cam_pos_ortho, cam_rays, init_zdepth, valid_mask, latent, march_step=None, clamp_dist=clamp_dist, no_grad=no_grad, use_transform=use_transform)
        elif ray_marching_type == 'trivial':
            cam_rays = self.ray_marching_recursive_ortho(R)
            return self.ray_marching_trivial_ortho(cam_pos_ortho, cam_rays, init_zdepth, valid_mask, latent, march_step=None, clamp_dist=clamp_dist, no_grad=no_grad, use_transform=use_transform)
        elif ray_marching_type == 'recursive':
#            cam_rays = self.get_camera_rays(R)
            cam_pos_ortho, cam_rays = self.get_rays_ortho(R)
            return self.ray_marching_recursive_ortho(cam_pos_ortho, cam_pos, cam_rays, init_zdepth, valid_mask, latent, march_step=None, clamp_dist=clamp_dist, no_grad=no_grad, use_transform=use_transform)
    ...
    def ray_marching_recursive_ortho(self, cam_pos_ortho, cam_pos, cam_rays, init_zdepth, valid_mask, latent, march_step=None, stop_threshold=None, clamp_dist=0.1, no_grad=False, use_transform=True, use_first_query_check=True):
        ...
        maxbound_zdepth = self.get_maxbound_zdepth_ortho(valid_cam_pos_ortho, cam_pos, valid_cam_rays)
        ...
        for i in range(march_step):
            ...
            valid_cam_pos_ortho_now = valid_cam_pos_ortho[:, unfinished_mask]
...
#        rays_end[1,:] = rays_end[1,:]*(-1)
#        rays_end[1,:] = rays.clone()
        return rays_start,  rays_end
    
#    
#    rays = torch.matmul(R.transpose(1,0), homo_calib) # (3, H*W)
#    rays_1 = sdf_renderer.normalize_vectors(rays) 
